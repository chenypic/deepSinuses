...
class EnDown122(nn.Module):
    def __init__(self, in_channels, out_channels):
        super(EnDown122, self).__init__()
        self.conv = nn.Conv3d(in_channels, out_channels, kernel_size=3, stride=(1,2,2), padding=(1,1,1))
...
class Unet(nn.Module):
    def __init__(self, in_channels=1, base_channels=32, num_classes=2):
        ...
        self.EnDown1 = EnDown122(in_channels=base_channels, out_channels=base_channels*2)
    ...
    def forward(self, x):
        x = self.InitConv(x)       # (1, 16, 64, 128, 128)

        x1_1 = self.EnBlock1(x)
        x1_2 = self.EnDown1(x1_1)  # (1, 32, 32, 64, 64)
        ...
        output = self.EnBlock4_4(x4_3)  # (1, 128, 8, 16, 16)
