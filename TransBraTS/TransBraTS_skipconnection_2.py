...
class IDH_network(nn.Module):
    def __init__(self):
        ...
        self.avg_pool_3d = nn.AvgPool3d((16,16,16), 1)
        self.max_pool_3d = nn.MaxPool3d((16,16,16), 1)
        self.Hidder_layer_1 = nn.Linear(1280, 512)
        self.Hidder_layer_2 = nn.Linear(512, 32)
        self.drop_layer = nn.Dropout(p=0.2)
...
class IDH_network_my(nn.Module):
    def __init__(self):
        super(IDH_network_my, self).__init__()
        self.avg_pool_3d = nn.AvgPool3d((16,16,16), 1)
        self.max_pool_3d = nn.MaxPool3d((16,16,16), 1)
        self.Hidder_layer_1 = nn.Linear(512, 128)

        self.clarelu = nn.ReLU()
        self.clarelu2 = nn.ReLU()
        self.Hidder_layer_2 = nn.Linear(128, 32)
        #self.drop_layer = nn.Dropout(p=0.2)
        self.classifier = nn.Linear(128, 2)
        #self.softmax = nn.Softmax(dim=1)

    def forward(self,x4_1,X_2_4_1):
        x = self.feature_fusion_layer(x4_1,X_2_4_1)
        #x = self.drop_layer(x)
        x = self.Hidder_layer_1(x)
        x = self.clarelu(x)
        #x = self.Hidder_layer_2(x)
    ...
    def feature_fusion_layer(self,x4_1,X_2_4_1):
        x4_1_avg = self.avg_pool_3d(x4_1)
        x4_1_max = self.max_pool_3d(x4_1)

        X_2_4_1avg = self.avg_pool_3d(X_2_4_1)
        X_2_4_1max = self.max_pool_3d(X_2_4_1)
        ...
        X_2_4_1avg = X_2_4_1avg.view(X_2_4_1avg.size(0), -1)
        X_2_4_1max = X_2_4_1max.view(X_2_4_1max.size(0), -1)

        return torch.cat([x4_1_avg,x4_1_max,X_2_4_1avg,X_2_4_1max], dim=1)
...
class Decoder_modual(nn.Module):
    def __init__(self):
        ...
        self.DeUp2 = DeUp_Cat122(in_channels=self.embedding_dim // 16, out_channels=self.embedding_dim // 32)
...
class Decoder_modual_my(nn.Module):
    def __init__(self):
        super(Decoder_modual_my, self).__init__()
...
class TransformerBraTS(nn.Module):
    ...
    def forward(self, x):
        ...
        x1_1, x2_1, x3_1,x4_1 = self.encode(x)
        return x1_1, x2_1, x3_1,x4_1
...
class DeUp_Cat122(nn.Module):
    def __init__(self, in_channels, out_channels):
        super(DeUp_Cat122, self).__init__()
        self.conv1 = nn.Conv3d(in_channels, out_channels, kernel_size=1)
        self.conv2 = nn.ConvTranspose3d(out_channels, out_channels, kernel_size=(1,2,2), stride=(1,2,2))
        self.conv3 = nn.Conv3d(out_channels*2, out_channels, kernel_size=1)
