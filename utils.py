...
def reload_ckpt_bis(ckpt, model_1, model_2, device=torch.device("cuda:0")):
    if os.path.isfile(ckpt):
        print(f"=> loading checkpoint {ckpt}")
        try:
            checkpoint = torch.load(ckpt, map_location=device)
            start_epoch = checkpoint['epoch']
            model_1.load_state_dict(checkpoint['en_state_dict'])
            model_2.load_state_dict(checkpoint['seg_state_dict'])
            return start_epoch
        except RuntimeError:
            # TO account for checkpoint from Alex nets
            print("Loading model Alex style")
