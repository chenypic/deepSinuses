...
class Random_Flip(object):
    def __call__(self, sample):
        ...
        return {'image': image, 'seg': seg, 'ids':sample['ids'],'label':sample['label']}
...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        #weight = cc2weight(cc)
        weight = 0
        return {'image': image, 'seg': seg,'weight':weight, 'ids':sample['ids'],'label':sample['label']}
...
class OPCData(Dataset):
    ...
    def __getitem__(self, item):
        ...
        label = int(list(self.data.iloc[[item]]['label'])[0])
        # ids = os.path.basename(os.path.dirname(list(self.data.iloc[[item]]['MRI'])[0]))

        ids = list(self.data.iloc[[item]]['ids'])[0]
            
        image = sitk.GetArrayFromImage(image)
        SEG = sitk.GetArrayFromImage(SEG)
            
        sample = {'image': image, 'seg': SEG, 'label':label, 'ids':ids }
