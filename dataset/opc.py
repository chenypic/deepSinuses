...
def get_datasets(seed, on="train", fold_number=0, normalisation="minmax"):
    ...
    # return patients_dir
    train_dataset = OPC(train, training=True,
                          normalisation=normalisation)
    val_dataset = OPC(val, training=False, data_aug=False,
                        normalisation=normalisation)
    bench_dataset = OPC(test, training=False, benchmarking=True,
                          normalisation=normalisation)
...
def get_test_datasets(seed, on="train", fold_number=0, normalisation="minmax"):
    ...
    bench_dataset = OPC(patients_dir, training=False, benchmarking=True,
                          normalisation=normalisation)
