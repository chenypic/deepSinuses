...
class MultiTaskLossWrapper(nn.Module):
    ...
    def forward(self, outputs,targets,weights):
        ...
        # seg_loss, loss1,loss2,loss3 = self.loss_fn[0](outputs[0], targets[0],weights[0]) # 原代码
        seg_loss, loss1 = self.loss_fn[0](outputs[0], targets[0],weights[0])
        ...
        loss = torch.mean(2*seg_loss_1+idh_loss_1) # 原代码

        #loss = 2*seg_loss+idh_loss

         # return loss,seg_loss,idh_loss,loss1,loss2,loss3,std_1,std_2,self.log_vars[0],self.log_vars[1] #原代码
        return loss,seg_loss,idh_loss,loss1,std_1,std_2,self.log_vars[0],self.log_vars[1]

class MultiTaskLossWrapper_seg(nn.Module):
    def __init__(self, task_num,loss_fn):
        super(MultiTaskLossWrapper_seg, self).__init__()
        self.task_num = task_num
        self.loss_fn = loss_fn

    def forward(self, outputs,targets,weights):
        seg_loss, loss1 = self.loss_fn[0](outputs[0], targets[0],weights[0])
        return seg_loss,seg_loss,loss1





class MultiTaskLossWrapper_seg_surv(nn.Module):
    def __init__(self, task_num,loss_fn):
        super(MultiTaskLossWrapper_seg_surv, self).__init__()
        self.task_num = task_num
        self.loss_fn = loss_fn
        self.log_vars = nn.Parameter(torch.tensor((0.0,0.0),requires_grad=True)) #1.0, 6.0
    ...
    def forward(self, outputs,targets,weights):
        ...
        surv_loss = self.loss_fn[1](targets[1],targets[2],outputs[1])

        surv_loss_1 = torch.sum(0.5 * torch.exp(-self.log_vars[1]) * surv_loss + self.log_vars[1],-1)

        loss = torch.mean(seg_loss+surv_loss) # 原代码

        #loss = 2*seg_loss+idh_loss

         # return loss,seg_loss,idh_loss,loss1,loss2,loss3,std_1,std_2,self.log_vars[0],self.log_vars[1] #原代码
        return loss,seg_loss,loss1, surv_loss,std_1,std_2,self.log_vars[0],self.log_vars[1]


class MultiTaskLossWrapper_T1_T2(nn.Module):
    def __init__(self, task_num,loss_fn):
        super(MultiTaskLossWrapper_T1_T2, self).__init__()
        self.task_num = task_num
        self.loss_fn = loss_fn
        self.log_vars = nn.Parameter(torch.tensor((0.0,0.0),requires_grad=True)) #1.0, 6.0

    def forward(self, outputs,targets, outputs2,targets2,weights):
        std_1 = torch.exp(self.log_vars[0]) ** 0.5
        std_2 = torch.exp(self.log_vars[1]) ** 0.5

        # seg_loss, loss1,loss2,loss3 = self.loss_fn[0](outputs[0], targets[0],weights[0]) # 原代码
        seg_loss, loss1 = self.loss_fn[0](outputs[0], targets[0],weights[0])
        T2_seg_loss, T2_loss1 = self.loss_fn[0](outputs2[0], targets2[0],weights[0])


        #seg_loss_, loss1,loss2,loss3 = softmax_dice(outputs[0], targets[0],weights[0])
        #seg_loss = self.loss_fn[0](outputs[0], targets[0])
        seg_loss_1 = torch.sum(0.5 * torch.exp(-self.log_vars[0]) * seg_loss + self.log_vars[0],-1) #
        T2_seg_loss_1 = torch.sum(0.5 * torch.exp(-self.log_vars[0]) * T2_seg_loss + self.log_vars[0],-1) #

        idh_loss = self.loss_fn[1](outputs[1], targets[1], weights[1])

        idh_loss_1 = torch.sum(0.5 * torch.exp(-self.log_vars[1]) * idh_loss + self.log_vars[1],-1)

        loss = torch.mean(seg_loss_1+T2_seg_loss_1 +idh_loss_1) # 原代码

        #loss = 2*seg_loss+idh_loss

         # return loss,seg_loss,idh_loss,loss1,loss2,loss3,std_1,std_2,self.log_vars[0],self.log_vars[1] #原代码
        return loss,seg_loss,T2_seg_loss,idh_loss,loss1,T2_loss1,std_1,std_2,self.log_vars[0],self.log_vars[1]
...
def CoxLoss(survtime, censor, hazard_pred):
    ...
    loss_cox = -torch.mean((theta - torch.log(torch.sum(exp_theta*R_mat, dim=1))) * (1-censor)) + 1
    return loss_cox
...
def softmax_dice(output, target,weight=None):
    ...
    if weight is None:
        loss1 = Dice(output[:, 1, ...], (target == 1).float())
        #loss2 = Dice(output[:, 2, ...], (target == 2).float()) # 二分割，屏蔽此代码
        # loss3 = Dice(output[:, 3, ...], (target == 3).float())  # 二分割，屏蔽此代码
        ...
        #weight_2[torch.where(target != 2)] = 1# 二分割，屏蔽此代码
        #weight_3[torch.where(target != 4)] = 1# 二分割，屏蔽此代码
        loss1_ = Dice(output[:, 1, ...], (target == 1).float(), weight_1)
        #loss2_ = Dice(output[:, 2, ...], (target == 2).float(), weight_2)# 二分割，屏蔽此代码
        #loss3_ = Dice(output[:, 3, ...], (target == 4).float(), weight_3)# 二分割，屏蔽此代码

        loss1 = Dice(output[:, 1, ...], (target == 1).float())
        #loss2 = Dice(output[:, 2, ...], (target == 2).float())# 二分割，屏蔽此代码
        #loss3 = Dice(output[:, 3, ...], (target == 4).float())# 二分割，屏蔽此代码
...
def idh_cross_entropy(input,target,weight=None):
