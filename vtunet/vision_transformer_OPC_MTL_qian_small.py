...
from .vt_unet_MTL_qian_small import SwinTransformerSys3D
...
class Surv_network_qian_snn_v4(nn.Module):
    def __init__(self):
        super(Surv_network_qian_snn_v4, self).__init__()
        self.avg_pool_3d = nn.AvgPool3d((8,8,8), 1)
        self.max_pool_3d = nn.MaxPool3d((8,8,8), 1)
        ...
        self.Hidder_layer_2 = nn.Linear(256, 64)
        self.relu2 = nn.ELU()
        self.drop_layer = nn.AlphaDropout(p=0.1, inplace=False)
        self.classifier = nn.Linear(64, 1)
        #self.softmax = nn.Softmax(dim=1)
