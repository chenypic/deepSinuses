...
class PatchExpand_Up(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.view(B, D, H, W, C)
        # x = rearrange(x, 'b d h w (p1 p2 c)-> b d (h p1) (w p2) c', p1=self.dim_scale, p2=self.dim_scale, c=C // 4)
        x = rearrange(x, 'b d h w (p3 p1 p2 c)-> b (d p3) (h p1) (w p2) c', p3=self.dim_scale, p1=self.dim_scale, p2=self.dim_scale, c=C // 8)
...
class PatchExpand(nn.Module):
    def __init__(self, input_resolution, dim, dim_scale=2, norm_layer=nn.LayerNorm):
        ...
        self.dim = dim
        # self.expand = nn.Linear(dim, 2 * dim, bias=False) if dim_scale == 2 else nn.Identity()
        self.expand = nn.Linear(dim, 4 * dim, bias=False) if dim_scale == 2 else nn.Identity() # 原来是2 * dim，改成4
        self.norm = norm_layer(dim // dim_scale)
    ...
    def forward(self, x):
        ...
        # x = x.view(B, D * 8, H, W, C) # 源代码
        x = x.view(B, D, H, W, C) # 改成不要乘以8，z轴也做了降采样
        # x = rearrange(x, 'b d h w (p1 p2 c)-> b d (h p1) (w p2) c', p1=self.dim_scale, p2=self.dim_scale, c=C // 4) #源代码

        #print('x的尺寸：',x.shape)
        
        x = rearrange(x, 'b d h w (p3 p1 p2 c)-> b (d p3) (h p1) (w p2) c', p3=self.dim_scale, p1=self.dim_scale, p2=self.dim_scale, c=C // 8) # 改
...
class SwinTransformerSys3D(nn.Module):
    ...
    def __init__(self, pretrained=None,
        ...
        self.patch_embed = PatchEmbed3D(img_size=img_size,
                                        ...
                                        norm_layer=norm_layer if self.patch_norm else None) # 首先切成不同的很多很多的小块
    ...
    def forward(self, x):
        x_bottle, x_downsample, v_values_1, k_values_1, q_values_1, v_values_2, k_values_2, q_values_2 = self.forward_features(
            x)

        # print('瓶颈层的尺寸：',x_bottle.shape)
        # print('每一层的尺寸：',x_downsample[0].shape)
        # print('每一层的尺寸：',x_downsample[1].shape)
        # print('每一层的尺寸：',x_downsample[2].shape)
        # print('每一层的尺寸：',x_downsample[3].shape)

        # print('v_values_1[0]的尺寸：',v_values_1[0].shape)
        # print('k_values_1[0]的尺寸：',k_values_1[0].shape)
        # print('q_values_1[0]的尺寸：',q_values_1[0].shape)

        # print('v_values_2[0]的尺寸：',v_values_2[0].shape)
        # print('k_values_2[0]的尺寸：',k_values_2[0].shape)

        
        x_out = self.forward_up_features(x_bottle, x_downsample, v_values_1, k_values_1, q_values_1, v_values_2, k_values_2,
                                     q_values_2)
        x_out = self.up_x4(x_out)

        return x_out,x_bottle
